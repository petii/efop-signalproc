...
int main(int argc, char **argv) {
  ...
  try {
    ...
    for (int multiplier = rangeBegin; multiplier <= rangeEnd; ++multiplier) {
      ...
      // std::clog << "Base window size:\t" <<
      // VisualizationApplication::baseWindowSize << std::endl; std::clog <<
      // "Running with a base window size multiplier of " << multiplier <<
      // std::endl; std::clog << std::string(50,'-') << std::endl; auto
      // initStart =
