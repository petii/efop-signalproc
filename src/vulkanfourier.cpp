...
void VulkanFourier::createInstance(
...
void VulkanFourier::copyDataToGPU(const std::vector<float>& data) {
    void *pData;
    vkMapMemory(
        device,
        bufferMemory,
...
std::vector<float> VulkanFourier::readDataFromGPU() {
void *pData;
    vkMapMemory(
        device,
        bufferMemory,
...
void VulkanFourier::runCommandBuffer() {
...
uint32_t VulkanFourier::getComputeQueueFamilyIndex(VkPhysicalDevice physicalDevice) {
...
void VulkanFourier::allocateBufferMemory(
    VkPhysicalDevice physicalDevice
) {
